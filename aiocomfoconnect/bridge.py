...
class Bridge:
    ...
    async def _process_message(self) -> None:
        ...
        try:
            ...
            if isinstance(exc, ComfoConnectNotAllowed):
                _LOGGER.error(f"Session not allowed. Details: {exc}")
                # Implement specific handling for NOT_ALLOWED response
            else:
                _LOGGER.error(f"Unexpected exception structure: {exc}")

            if hasattr(exc, 'message') and isinstance(exc.message, Message):
                # Assuming Message has a cmd attribute with a reference
                if hasattr(exc.message.cmd, 'reference'):
                    self._event_bus.emit(exc.message.cmd.reference, exc)
                else:
                    _LOGGER.error(f"Exception message does not have a cmd.reference: {exc}")
            else:
                _LOGGER.error(f"Unexpected exception structure: {exc}")
